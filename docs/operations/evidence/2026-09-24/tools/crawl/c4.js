...
const own=h=>HOSTS.has(h)||/\.public\.blob\.vercel-storage\.com$/.test(h);const extA=new Map(),cand=[];
for(const [u,v] of am){let h;try{h=new URL(u).hostname}catch(e){continue}const social=[...v.kinds].some(k=>/og:image|twitter:image/.test(k));if(own(h)||social)cand.push({u,v,social});else extA.set(h,(extA.get(h)||0)+1)}
cand.sort((a,b)=>b.social-a.social);D.assetRefs=am.size;D.assetCand=cand.length;D.extAssetHosts=extA;const list=cand.slice(0,MAXASSETS);D.assetCapLeft=cand.length-list.length;
...
["vercel-domain","https://mayaallan.vercel.app/"],["vercel-domain","https://mayaallan.vercel.app/about"],["vercel-domain","https://mayaallan-mallan.vercel.app/"],["vercel-domain","https://mayaallan-git-main-mallan.vercel.app/"],
...
const UAS={googlebot:"Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)",bingbot:"Mozilla/5.0 (compatible; bingbot/2.0; +http://www.bing.com/bingbot.htm)",
gptbot:"Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko); compatible; GPTBot/1.1; +https://openai.com/gptbot",claudebot:"Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko; compatible; ClaudeBot/1.0; +claudebot@anthropic.com)",
perplexitybot:"Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko; compatible; PerplexityBot/1.0; +https://perplexity.ai/perplexitybot)",facebook:"facebookexternalhit/1.1 (+http://www.facebook.com/externalhit_uatext.php)",
chrome:"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36",audit:UA};
const combos=[];for(const pth of ["/","/faq","/media","/privacy"])for(const n of Object.keys(UAS))combos.push([pth,n]);
D.ua=await pool(combos,CONC,async([pth,n])=>{const r=await req(M+pth,{ua:UAS[n]});const p=r.body?parse(r.body,r.final):null;
return {pth,n,status:r.status,at:r.at,t:r.t,len:Buffer.byteLength(r.body||""),titleInHead:!!(p&&p.titleInHead),descInHead:!!(p&&p.descInHead),metaInHead:p?p.metaInHead:0,metaTotal:p?p.metaTotal:0,title:p&&p.title,canon:p&&p.canon,og:p?p.ogImg.length:0,ld:p?p.ld.length:0,words:p?p.words:0,cache:r.h["x-vercel-cache"]||""}});
D.end=now();return D}